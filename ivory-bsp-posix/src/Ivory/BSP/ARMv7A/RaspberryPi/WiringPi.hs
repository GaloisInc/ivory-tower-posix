{-|
Module      : Ivory.BSP.ARMv7A.RaspberryPi.WiringPi
Description : This module imports all usefull functions from the wiringPi library for use with Raspberry Pi
Copyright   : (c) WiringPi
License     : LGPL-3
Maintainer  : smaccm@galois.com
Stability   : experimental

Documentation taken from the wiringPi documentation. Please see <http://wiringpi.com/reference/ here> for more details.
-}
...
module Ivory.BSP.ARMv7A.RaspberryPi.WiringPi 
  ...
  wiringPi_header,
  softTone_header,
  -- * TYPES DEFINED FOR 32 BITS ARCHITECTURES
  CSize_t,
  CInt,

  -- * SETUP FUNCTIONS
  -- | Please see <http://wiringpi.com/reference/setup/> for more details.
  wiringPiSetup,
  wiringPiSetupSys,
  wiringPiSetupGpio,
  wiringPiSetupPhys,
  ...
  -- * CORE 
  -- | Please see <http://wiringpi.com/reference/core-functions/> for more details.

  -- ** CORE DEFINITIONS
  wp_INPUT, wp_OUTPUT, wp_PWM_OUTPUT, wp_GPIO_CLOCK,
  wp_PUD_OFF, wp_PUD_DOWN, wp_PUD_UP,
  wp_HIGH, wp_LOW,
...
-- | Defining the size_t type as Uint32
type CSize_t = Uint32

-- | Defining the Int type as Sint32
type CInt    = Sint32
...
-- | Setup the interface with /sys/class/gpio instead of accessing hardware directly. 
--   Note that you can access only pins that have been exported first.
...
-- | Same as setup gpio, but allows access only to P1 connector (see <http://elinux.org/RPi_Hardware>)
...
-- 
-- > void softToneWrite (int pin, int value)
...
-- | The ModuleDef exported by the module. Add this moduledef 
--   in the dependencies of a Tower program
